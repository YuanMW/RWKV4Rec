...
class GPT(nn.Module):
    ...
    def forward(self, idx,pos_seq,neg_seq):
        B, T = idx.size()
        ...
        pos_embs = self.tok_emb(pos_seq) #[Batch,seq,768]
        neg_embs = self.tok_emb(neg_seq) #[Batch,seq,768]
        print("x shape:{}".format(x.shape))
        print("pos embedding shape:{}".format(pos_embs.shape))
        print("neg embedding shape:{}".format(neg_embs.shape))
        pos_logits = (x * pos_embs).sum(dim=-1)
        neg_logits = (x * neg_embs).sum(dim=-1)
        return x