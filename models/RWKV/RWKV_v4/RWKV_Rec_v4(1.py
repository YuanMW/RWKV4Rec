...
wkv_cuda = load(name="wkv", sources=['models/RWKV/RWKV_v4/cuda/wkv_op.cpp', 'models/RWKV/RWKV_v4/cuda/wkv_cuda.cu'],
                verbose=True,
...
def RWKV_Init(model, item_num, args):  # fancy initialization of all lin & emb layer in the model
...
class RWKV_TimeMix(torch.jit.ScriptModule):
    def __init__(self, args, layer_id, n_layer=6):
        ...
        self.key = nn.Linear(args.hidden_units, attn_sz, bias=False)
        self.value = nn.Linear(args.hidden_units, attn_sz, bias=False)
        self.receptance = nn.Linear(args.hidden_units, attn_sz, bias=False)

        self.output = nn.Linear(attn_sz, args.hidden_units, bias=False)
...
class GPT(nn.Module):
    def __init__(self,
        ...
        self.head = nn.Linear(args.hidden_units, self.maxlen, bias=False)

        if RWKV_HEAD_QK_DIM > 0:
            self.head_q = nn.Linear(args.hidden_units, RWKV_HEAD_QK_DIM, bias=False)
            self.head_q.scale_init = 0
            self.head_k = nn.Linear(args.hidden_units, RWKV_HEAD_QK_DIM, bias=False)
    ...
    def GPT(self, log_seqs, targets=None):
        ...
        if RWKV_HEAD_QK_DIM > 0:
            ...
            if '32' in os.environ['RWKV_FLOAT_MODE']:
                c = c @ F.one_hot(x, num_classes=self.item_num)
            elif os.environ['RWKV_FLOAT_MODE'] == 'fp16':
                c = c @ F.one_hot(x, num_classes=self.item_num).half()
            elif os.environ['RWKV_FLOAT_MODE'] == 'bf16':
                c = c @ F.one_hot(x, num_classes=self.item_num).bfloat16()
