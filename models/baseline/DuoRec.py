...
class DuoRec(nn.Module):
    ...
    def predict(self, user_ids, log_seqs, item_indices):
        """推理前向传播"""
        log_feats = self.log2feats(log_seqs)
        final_feat = log_feats[:, -1, :]  # 使用最后一个位置
    ...
    def calculate_contrastive_loss(self, original_seq, aug_seq=None, sem_aug_seq=None):
        ...
        original_output = original_feats[:, -1, :]  # 使用最后一个位置
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self, args):
        ...
        self.self_attn = nn.MultiheadAttention(
            ...
            batch_first=True  # 使用batch_first简化操作
        )
